...
export async function subscribeToPushNotifications(activeRole) {
  try {
    ...
    console.log(
      `[PushService] Calling updateUserPushToken with role ${activeRole} for endpoint:`,
      subscriptionObject.endpoint,
    );
    const success = await updateUserPushToken(
      supabase,
      user.id,
      subscriptionObject,
    );
...
export async function triggerStaffPushNotification(
  type,
  restaurantId,
  tableNumber,
) {
  ...
  try {
    console.log(
      `[PushService] Triggering '${type}' notification for Table ${tableNumber} (Restaurant ${restaurantId})...`,
    );
    ...
    if (res.ok) {
      console.log(
        `[PushService] Successfully triggered '${type}' notification API.`,
      );
    } else {
      console.error(
        `[PushService] Failed to trigger notification API. Status: ${res.status}`,
      );
    }
  } catch (error) {
    console.error(
      `[PushService] Fetch Error when triggering ${type} push notification:`,
      error,
    );
  }
}
