...
export default function KitchenSummaryBar({
  orders,
  isOpen,
  onClose,
  isLoggingOut,
      ...
      if (["pending", "confirmed", "preparing"].includes(order.status)) {
        const title =
          typeof order.products?.title === "object"
            ? order.products.title?.[language] ||
              order.products.title?.[fallbackLang] ||
              "Unknown"
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar w-full flex flex-col items-center gap-6 py-2">
          {summary.length === 0 ? (
            ...
            summary.map((item, idx) => (
              <div
                key={idx}
                className="flex flex-col items-center gap-2 group relative px-2"
              >
