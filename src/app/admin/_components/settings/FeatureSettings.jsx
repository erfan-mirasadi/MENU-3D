...
import { updateRestaurantById } from "@/services/restaurantService";
...
export default function FeatureSettings() {
  ...
  const handleToggle = (key) => {
    setFeatures((prev) => {
      ...
      if (key === "waiter" || key === "cashier" || key === "kitchen") {
        const anyAppEnabled =
          newState.waiter || newState.cashier || newState.kitchen;
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await updateRestaurantById(restaurant.id, { features });
  ...
  return (
    <div className="bg-dark-700 rounded-2xl p-6 border border-gray-800">
      ...
      <div className="flex justify-end">
        <button
          onClick={handleSave}
          disabled={loading}
          className="px-6 py-3 bg-accent hover:bg-accent-hover text-white font-bold rounded-xl flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-lg shadow-orange-900/20 active:scale-95"
        >
          {loading ? (
            <Loader active={true} variant="inline" className="w-4 h-4" />
          ) : (
            <FaSave />
          )}
          Save Changes
  ...
  return (
    <button
      ...
      className={`flex items-start gap-4 p-4 rounded-xl border transition-all text-left ${
        active
          ? "bg-dark-800 border-accent shadow-[0_0_15px_rgba(234,124,105,0.1)]"
          : "bg-dark-800/50 border-gray-800 opacity-80 hover:opacity-100"
      } ${disabled ? "opacity-40 cursor-not-allowed" : ""}`}
    >
      <div
        className={`p-3 rounded-lg bg-dark-700 ${active ? color : "text-gray-500"}`}
      >
