      ...
      <div className="flex justify-between items-center mb-6 relative">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 text-white border border-[#393C49] px-4 py-2 rounded-lg text-sm bg-[#1F1D2B] hover:bg-dark-800 cursor-pointer"
          >
            <svg
              width="16"
              height="16"
          ...
          {showFilterMenu && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-dark-800 border border-[#393C49] rounded-lg shadow-lg z-10 py-1">
              {[
                "All",
                "Pending",
                "Preparing",
          ...
          <tbody className="text-neutral-300 font-medium text-sm">
            {loading ? (
              ...
              <>
                {(isExpanded
                  ? displayedOrders
                  : displayedOrders.slice(0, 6)
                ).map((order) => (
                  <tr
                    key={order.id}
                    className="border-b border-[#393C49] hover:bg-dark-800/50 transition-colors"
                  >
                    <td className="py-4 pl-4 text-[#E0E6E9] font-bold">
                      {order.tableNo}
                    </td>
                    <td className="py-4 text-[#E0E6E9]">
                      {content(order.menu)}
                    </td>
                    <td className="py-4 text-[#E0E6E9]">
