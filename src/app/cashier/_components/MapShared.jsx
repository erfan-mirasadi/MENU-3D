...
export function MapCamera() {
  return <PerspectiveCamera makeDefault fov={10} position={[-60, 80, 100]} />;
}
...
export function MapFloor({ textureType = "terrazzo" }) {
  ...
  if (textureType === "white") {
    return (
      ...
      >
        ...
        <meshStandardMaterial color="dark-800" roughness={0.1} />
      </mesh>
    );
  }
  ...
  const roughness =
    textureType === "concrete" ? 0.9 : textureType === "marble" ? 0.1 : 0.8;
...
export function getFloorColor(type) {
  switch (type) {
    case "parquet":
      return "#d4c5b0"; // Match wood tone
    case "concrete":
      return "#808080"; // Match concrete grey
    case "marble":
      return "#e5e7eb"; // Match white marble
    case "terrazzo":
      return "#f3f4f6"; // Match terrazzo background
    case "black":
      return "#1f1d2b";
    default:
      return "#f3f4f6";
...
// Shared Table Visuals (Body + Legs + Text)
