...
import { createTable, updateTableLayout } from "@/services/tableService";
...
export default function DashboardPage() {
  ...
  const [sourceTableIdForTransfer, setSourceTableIdForTransfer] =
    useState(null);
  ...
  const mergedTables = useMemo(() => {
    const merged = tables.map((table) => {
      ...
      //Normal Visuals
        ...
        // Bill/Service Request (Red)
        const hasPaymentRequest = requests.some(
          (r) => r.status === "pending" && r.request_type === "bill",
        );
  ...
  const handleNormalTableClick = async (tableId) => {
    // Check if Transfer Mode
    if (isTransferMode) {
      handleTableSelection(tableId);
      return;
    }

    //Check for Bill Request (Resolve on Click)
    const table = mergedTables.find((t) => t.id === tableId);
    if (table?.status === "payment_requested") {
      const pendingRequests = table.session?.service_requests?.filter(
        (r) => r.status === "pending",
      );
  ...
  const handleTableSelection = async (targetId) => {
    ...
    const sourceSession = sessions.find(
      (s) => s.table_id === sourceTableIdForTransfer,
    );
    if (!sourceSession) {
    ...
    if (isMerge) {
      if (
        !confirm(
          `Merge Table ${sourceTable.table_number} into Table ${targetTable.table_number}?`,
        )
      )
        return;
      ...
      if (
        !confirm(
          `Move Table ${sourceTable.table_number} to Table ${targetTable.table_number}?`,
        )
      )
        return;
  ...
  const handleAddTable = async () => {
    ...
    const usePrefix =
      ...
      localTables.some((t) => t.table_number.toString().startsWith("T-"));
    const nextTableNumber = usePrefix
      ? `T-${String(nextNum).padStart(2, "0")}`
      : nextNum.toString();
    const finalTableNumber =
      localTables.length === 0 ? "T-01" : nextTableNumber;
  ...
  const handleResetLayout = () => {
    if (
      !confirm(
        "Are you sure? This will strictly arrange tables by number and reset all custom sizes.",
      )
    )
      return;
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-[#1F1D2B]">
      ...
      {isTransferMode && (
        <div className="absolute bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 z-50 bg-accent text-white px-6 md:px-8 py-3 md:py-4 rounded-full shadow-2xl animate-in slide-in-from-bottom flex gap-4 md:gap-6 items-center">
          <div>
            <p className="font-bold text-sm md:text-lg whitespace-nowrap">
              {t("selectTargetTable")}
            </p>
      ...
      <div className="absolute inset-0 z-10 pointer-events-none px-3 pb-3 px-[env(safe-area-inset-left)] pt-[calc(env(safe-area-inset-top)+0.75rem)] md:px-6 md:pb-6 md:pt-[calc(env(safe-area-inset-top)+1.5rem)] flex flex-col justify-between">
        ...
        <header className="flex flex-wrap justify-between items-start gap-2 pointer-events-auto">
          <div className="flex items-start flex-col gap-1">
            <h1 className="text-lg md:text-2xl font-bold tracking-wider uppercase text-white drop-shadow-md select-none">
              {t("floorManager")}
            </h1>
            {isEditing && (
              <span className="text-xs bg-blue-500/20 text-blue-200 px-2 py-0.5 rounded border border-blue-500/30 font-bold">
                {t("editMode")}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-wrap justify-end">
            ...
            {!isEditing && (
              <>
                <div className="hidden md:flex bg-dark-800/90 backdrop-blur-md px-4 py-2 rounded-xl shadow-lg border border-white/10 text-sm font-medium items-center gap-3 text-white select-none">
                  {/* System Status */}
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-300">
                      {isConnected ? t("systemOnline") : t("offline")}
                    </span>
                  </div>
                  <div className="w-px h-4 bg-white/10"></div>

                  {/* 1. Bill Requests (Blue Blink) - High Priority */}
                  {mergedTables.filter((t) => t.status === "payment_requested")
                    .length > 0 && (
                    <>
                  ...
                  {/* Served */}
                  {mergedTables.filter((t) => t.status === "active").length >
                    0 && (
                    <>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-green-200">
                          {
                            mergedTables.filter((t) => t.status === "active")
                              .length
                          }{" "}
                          Served
                        </span>
                  ...
                  {/* Kitchen/Preparing */}
                  {mergedTables.filter((t) => t.status === "preparing").length >
                    0 && (
                    <>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-yellow-200">
                          {
                            mergedTables.filter((t) => t.status === "preparing")
                              .length
                          }{" "}
                          {t("kitchenStatus")}
                        </span>
                  ...
                  {/* To Confirm */}
                  {mergedTables.filter((t) => t.status === "confirmed").length >
                    0 && (
                    <>
                      <div className="flex items-center gap-2 animate-pulse">
                        ...
                        <span className="text-orange-200">
                          {
                            mergedTables.filter((t) => t.status === "confirmed")
                              .length
                          }{" "}
                          To Confirm
                        </span>
                  ...
                  {/* Pending */}
                  {mergedTables.filter((t) => t.status === "kitchen_sent")
                    .length > 0 && (
                    <>
                      <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-300">
                      {mergedTables.filter((t) => t.status !== "free").length}{" "}
                      {t("occupied")}
                    </span>
                  </div>
                </div>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold uppercase transition-all border  ${
                        sortingMode === "priority"
                          ...
                          : "bg-dark-800 border-white/10 text-gray-400 hover:bg-white/5"
                      } cursor-pointer`}
                    >
                      <FaList />
                      {sortingMode === "priority"
                ...
                (isEditing ? (
                  <div className="flex gap-2 flex-wrap animate-in slide-in-from-top-2">
                    <button
                      ...
                      className="bg-dark-800 text-green-400 border border-green-500/30 px-4 py-2 rounded-xl shadow-lg font-bold flex items-center gap-2 hover:bg-[#2d3042] hover:border-green-500 hover:text-green-300 transition-all mr-2 cursor-pointer"
                    >
                    ...
                    <div className="relative group mr-2">
                      <select
                        ...
                        className="bg-dark-800 text-gray-200 border border-white/10 px-4 py-2 rounded-xl shadow-lg font-bold flex items-center gap-2 hover:bg-[#2d3042] hover:border-white/30 transition-all appearance-none pr-8 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 z-100"
                      >
                    ...
                    <button
                      ...
                      className="bg-dark-800 text-red-500 border border-red-500/30 px-4 py-2 rounded-xl shadow-lg font-bold flex items-center gap-2 hover:bg-[#2d3042] hover:border-red-500 hover:text-red-400 transition-all mr-4 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="bg-dark-800 text-gray-400 border border-white/10 px-4 py-2 rounded-xl shadow-lg font-bold flex items-center gap-2 hover:bg-[#2d3042] hover:text-white transition-all cursor-pointer"
                    >
                  ...
                  <button
                    ...
                    className="bg-dark-800 text-gray-200 border border-white/10 px-4 py-2 rounded-xl shadow-lg font-bold flex items-center gap-2 hover:bg-[#2d3042] hover:border-white/30 hover:text-white transition-all cursor-pointer"
                  >
            ...
            {!isEditing && (
              <div className="flex items-center gap-2">
                <div className="bg-dark-800/90 backdrop-blur-md p-1 rounded-xl shadow-lg border border-white/10 flex gap-1">
                  <button
                ...
                <button
                  ...
                  className="md:hidden bg-dark-800    /90 backdrop-blur-md p-2 rounded-xl shadow-lg border border-white/10 text-red-400 hover:bg-red-500/20 active:scale-90 transition-all disabled:opacity-50 min-w-[36px] min-h-[36px] flex items-center justify-center"
                  title="Logout"
                >
                  {isLoggingOut ? (
