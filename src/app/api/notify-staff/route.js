  ...
  try {
    ...
    if (!notificationConfig) {
      return NextResponse.json(
        { message: "No push notification required for this event type." },
        { status: 200 },
      );
    }

    const { targetRoles, payload } = notificationConfig(tableNumber);
    const staff = await getRestaurantStaffPushTokens(
      supabase,
      restaurantId,
      targetRoles,
    );
    ...
    if (!staff?.length) {
      return NextResponse.json(
        { message: "No subscribed staff found." },
        { status: 200 },
      );
    }
      ...
      staff.flatMap((user) => {
        ...
        return subscriptions.map(async (subscription) => {
          if (subscription.activeRole) {
            ...
            const isOwnerCatchAll =
              targetRoles.includes("owner") &&
              subscription.activeRole === "owner";
          ...
          try {
            ...
            console.log(
              `[API /notify-staff] 🚀 Sent push to User ${user.id} on endpoint: ...${subscription.endpoint.slice(-20)}`,
            );
          } catch (err) {
            // 404/410 means the subscription is expired or revoked — remove it from the array.
            if (err.statusCode === 404 || err.statusCode === 410) {
              console.warn(
                `[API /notify-staff] ❌ Stale token for User ${user.id}. Removing endpoint.`,
              );
              ...
              console.error(
                `[API /notify-staff] ⚠️ Error sending push to User ${user.id}:`,
                err.message,
              );
            }
          }
