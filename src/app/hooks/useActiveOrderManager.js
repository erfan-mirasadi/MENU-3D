...
export const useActiveOrderManager = (session, table, onCheckout, role = "waiter", onCloseDrawer, onRefetch) => {
  ...
  const {
      ...
      localItems, pendingItems, confirmedItems, activeItems, totalAmount,
  ...
  const handlePaymentRequest = async () => {
    if (onRefetch) {
        ...
        try { await onRefetch(); } 
        catch(e) { console.error("Failed to refetch session", e); } 
  ...
  return {
    ...
    actions: {
        ...
        handleConfirmOrder: () => dispatchOps.handleConfirmOrder(confirmedEdits, setConfirmedEdits),
        handleStartPreparing: () => dispatchOps.handleStartPreparing(confirmedEdits, setConfirmedEdits),
