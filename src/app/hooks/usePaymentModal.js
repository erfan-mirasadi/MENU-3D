...
import {
  getOrCreateBillLogic,
  addBillAdjustmentLogic,
...
export default function usePaymentModal({
  ...
  const [selectedAdjustmentIndices, setSelectedAdjustmentIndices] = useState(
    new Set(),
  );
  ...
  const orderItems = useMemo(() => {
    return (
      session?.order_items?.filter((item) =>
  ...
  const totalOrderAmount = useMemo(() => {
    return orderItems.reduce(
      (acc, item) => acc + item.quantity * Number(item.unit_price_at_order),
      0,
    );
  }, [orderItems]);
  ...
  }, [isOpen, session?.id, bill?.id, remainingTotal]);
  ...
  const amountToPay = useMemo(() => {
    ...
    if (splitMode === "ITEMS") {
      ...
      const selectedUnpaid = unpaidItems.filter((i) =>
      ...
      const allUnpaidSelected =
        unpaidItems.length > 0 && selectedUnpaid.length === unpaidItems.length;
    ...
    try {
      ...
      if (paymentMethod === "MIXED") {
        ...
        if (Math.abs(totalMixed - amountToPay) > 0.009) {
          toast.error(
            `Error: Total mixed payment (${totalMixed.toFixed(2)}) must exactly match the amount due (${amountToPay.toFixed(2)}).`,
          );
          setProcessing(false);
          return;
        }
        ...
        if (activeTab === "FULL") {
          ...
          itemsToRecord = itemsWithStatus.filter((i) =>
            selectedItemIds.has(i.id),
          );
        }
      ...
      console.log(
        `[PaymentModal] Payment Recorded. Session=${session.id}, Amount=${amountToPay}, Method=${paymentMethod}, FullyPaid=${res.fullyPaid}`,
      );
  ...
  return {
    ...
    setSelectedItemIds,
    selectedAdjustmentIndices,
    setSelectedAdjustmentIndices,
    customAmount,
    setCustomAmount,
    paymentMethod,
    setPaymentMethod,
    mixedCash,
    setMixedCash: handleMixedCashChange,
    mixedCard,
    setMixedCard: handleMixedCardChange,
    showAdjModal,
    setShowAdjModal,
    adjData,
    setAdjData,
