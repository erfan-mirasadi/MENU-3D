    ...
    <div className="receipt-container hidden">
      <style jsx global>{`
        @media print {
          ...
          .receipt-container {
            ...
            color: black;
            font-family:
              "Courier New", Courier, monospace; /* Monospace is best for receipts */
            font-size: 12px;
            line-height: 1.2;
            padding: 0;
            margin: 0;
      ...
      <div className="max-w-[80mm] mx-auto p-4 bg-white text-black">
        ...
        <div className="text-center mb-4">
          ...
          <div className="text-xs uppercase wrap-break-word">
            ...
            <div key={`${item.id}-${idx}`} className="text-xs flex items-start">
              ...
              <div className="flex-1">
                <div className="font-semibold">
                  {getLocalized(item.product?.title) || "Unknown Item"}
                </div>
                {item.selected_variants &&
                  Object.values(item.selected_variants).length > 0 && (
                    <div className="text-[10px] text-gray-600 pl-1">
                      {Object.values(item.selected_variants)
                        .map((v) => getLocalized(v.name))
                        .join(", ")}
                    </div>
                  )}
              </div>
