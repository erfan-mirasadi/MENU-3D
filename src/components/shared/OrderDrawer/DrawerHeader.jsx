...
export default function DrawerHeader({
  ...
  return (
    <div className="p-4 bg-dark-800 border-b border-white/5 flex flex-col gap-4 shadow-md z-10 shrink-0">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-white tracking-tight">
            {t("table")}{" "}
            <span className="text-accent">{table.table_number}</span>
          </h2>
          <span
          ...
          {session && (
            <>
              ...
              <button
                onClick={onOpenMenu}
                disabled={loading}
                className="h-10 px-4 bg-accent text-white rounded-lg flex items-center gap-2 font-bold text-sm shadow-lg shadow-orange-900/20 active:scale-95 transition-transform cursor-pointer disabled:opacity-50 disabled:pointer-events-none"
              >
                <FaPlus /> {t("add")}
              </button>
      ...
      {session && <NoteSection session={session} />}
    </div>
  );
}
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-white/90 text-sm leading-relaxed whitespace-pre-wrap">
            {session.note || (
              <span className="text-white/20 italic">{t("noNotes")}</span>
            )}
          </p>
        </div>
        <button
