...
export default function ServiceButtons({ restaurantId, tableId, sessionId }) {
  ...
  return (
    <>
      {/* Backdrop Overlay */}
      <div
        className={`fixed inset-0 bg-black/60 backdrop-blur-md transition-opacity duration-500 z-9999 ${confirming ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setConfirming(null)}
      />

      <div
        className={`relative flex flex-col gap-4 transition-all duration-500 z-9999 ${confirming ? "translate-x-4" : ""}`}
      >
        {/* Call Waiter Button */}
        {showWaiter && (
          <div
            className={`relative flex items-center transition-all duration-500 ${confirming === "call_waiter" ? "w-64" : "w-12"}`}
          >
            <button
              onClick={() => handleClick("call_waiter")}
              disabled={
                loading === "call_waiter" ||
                (confirming && confirming !== "call_waiter")
              }
              className={`
                        group relative flex items-center overflow-hidden
                        bg-white/10 backdrop-blur-md border border-white/20 shadow-xl 
                        transition-all duration-500 ease-out
                        ${confirming === "call_waiter" ? "w-full rounded-2xl bg-accent border-accent pl-3" : "w-11 h-11 rounded-full justify-center hover:bg-white/20 active:scale-95"}
                        ${confirming && confirming !== "call_waiter" ? "opacity-0 scale-0 h-0 w-0 border-0 p-0 m-0 overflow-hidden" : ""}
                    `}
              aria-label="Call Waiter"
            >
              <div
                className={`flex items-center justify-center shrink-0 ${confirming === "call_waiter" ? "" : "w-11 h-11 "}`}
              >
                <FaConciergeBell
                  className={`text-white text-xl ${loading === "call_waiter" ? "animate-pulse" : "group-hover:animate-wiggle"}`}
                />
              </div>
        ...
        {showBill && (
          <div
            className={`relative flex items-center transition-all duration-500 ${confirming === "bill" ? "w-64" : "w-12"}`}
          >
            <button
              onClick={() => handleClick("bill")}
              disabled={
                loading === "bill" || (confirming && confirming !== "bill")
              }
              className={`
                        group relative flex items-center overflow-hidden
                        bg-white/10 backdrop-blur-md border border-white/20 shadow-xl 
            ...
            >
              <div
                className={`flex items-center justify-center shrink-0 ${confirming === "bill" ? "" : "w-11 h-11"}`}
              >
                <FaFileInvoiceDollar
                  className={`text-white text-xl ${loading === "bill" ? "animate-pulse" : ""}`}
                />
              </div>
